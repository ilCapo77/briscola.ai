...
- Asse sperimentale successivo: usare PIMC come **teacher offline** e distillare solo le correzioni di **search** in un
  ...
  - aggiornata diagnostica teacher: `PIMCAgent` espone per ogni decisione search valori medi per carta, margine
    ...
    misurare se esistono davvero `15k..30k` disaccordi search ad alto margine/affidabilità. Run diagnostico `175k`,
    `64` determinizzazioni, `u=8`, seed `20260630`: `79,546` search, `95,454` solver, `37,657` search in disaccordo
    con v6; `14,978` search forti+affidabili con soglia `margin>=2` e CI low `>=0` (`10,505` se CI low `>=2`).
    Segnale presente ma non abbondante: siamo al minimo utile per un primo fit test/training pesato, non ancora a una
    massa ampia da `30k` correzioni.
