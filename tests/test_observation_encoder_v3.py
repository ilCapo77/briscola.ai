...
def test_fast_and_numba_encoders_reject_v3() -> None:
    """Guard domain-first: v3 deve fallire con errore chiaro su fast/numba, senza fallback a v2."""
    state = new_fast_2p_state(seed=1)
    seen = tuple(0 for _ in range(40))

    with pytest.raises(ValueError, match="v3 non supportato sul path fast"):
